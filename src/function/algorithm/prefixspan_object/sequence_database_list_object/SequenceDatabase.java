package function.algorithm.prefixspan_object.sequence_database_list_object;
...
*/
import function.algorithm.prefixspan_object.MyComparator;
...
public class SequenceDatabase<T> {
	...
	private final List<Sequence<T>> sequences = new ArrayList<Sequence<T>>();
	 ...
	 */
	public void loadFileWithItemString(String path) throws IOException {
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				if (thisLine.isEmpty() == false &&
					...
					addSequenceWithItemString(thisLine.split(" "));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @param tokens A list of tokens from the line (which were separated by spaces in the original file).
         * each token has format: String
	 */
	void addSequenceWithItemString(String[] tokens) { 
		// create a new Sequence to store the sequence
		Sequence<T> sequence = new Sequence<>(sequences.size());
		// create a list of strings for the first itemset.
		List<String> itemset = new ArrayList<String>();
		// for each token in this line: check token is item???
		for (String token : tokens) {
			...
			// if the token is -1, it means that we reached the end of an itemset.
				...
				Collections.sort(itemset, new MyComparator<String>());
				// we add the current itemset to the sequence
				sequence.addItemset((List<T>)itemset);
	...
	public String toString() {
		...
		for (Sequence<T> sequence : sequences) { // pour chaque objet
