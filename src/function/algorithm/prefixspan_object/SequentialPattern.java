 ...
 */
public class SequentialPattern<T>{
	/** list of itemsets */
	private final List<Itemset<T>> itemsets = new ArrayList<>();
	 ...
	 */
	 void addItemset(Itemset<T> itemset) {
	 ...
	 */
	public List<Itemset<T>> getItemsets() {
		return itemsets;
	}
	 ...
	 * @return the itemset
	 */
	public Itemset<T> get(int index) {
	...
	SequentialPattern cloneSequenceMinusItems(Map<T, Set<Integer>> mapSequenceID, double relativeMinSup) {
		SequentialPattern sequence = new SequentialPattern(getId());
		for(Itemset<T> itemset : itemsets){
			Itemset<T> newItemset = itemset.cloneItemSetMinusItems(mapSequenceID, relativeMinSup);
