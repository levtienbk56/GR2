...
class PseudoSequence<T> {
	...
	protected int size() {
		...
		if(size == 1 && ((List<T>)sequence.getItemsets().get(firstItemset)).size() == 0){
			return 0;
		}
		// return the size
		return size;
	...
	protected int getSizeOfItemsetAt(int index) {
		...
		//   index + firstItemset.
		int size = ((List<T>)sequence.getItemsets().get(index + firstItemset)).size();
	 ...
	 * @return the item.
	 */
	protected T getItemAtInItemsetAt(int indexItem, int indexItemset) {
	 ...
	 * @return the itemset
	 */
	protected List<T> getItemset(int index) {
	 ...
	 */
	protected int indexOf(int indexItemset, T idItem) {
